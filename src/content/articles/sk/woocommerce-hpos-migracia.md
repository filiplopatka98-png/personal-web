...
Ak máš WooCommerce eshop založený pred rokom 2023, tvoje objednávky s vysokou pravdepodobnosťou ešte stále bývajú tam, kde bývali od začiatku WordPressu — v tabuľke `wp_posts` spolu s článkami a stránkami, a metadáta k nim v `wp_postmeta`. Funguje to, ale je to architektonicky zlá dohoda: `wp_postmeta` je EAV štruktúra (key-value páry), takže aj jednoduchý filter „objednávky za posledný mesiac so statusom completed" znamená viacero JOINov na tabuľku, ktorá rastie donekonečna a nemá poriadne indexy pre biznisové otázky, ktoré si eshop kladie.
